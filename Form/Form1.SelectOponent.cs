...
namespace Banana_Chess
{
    internal partial class Form1
    {
        ...
        private InvitationOptions defaultInvOptions = new InvitationOptions();
        ...
        private void canvas_Paint_SendInv(object sender, PaintEventArgs e)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                e.Graphics.DrawImage(defaultInvOptions.getAtIndex(i) ? invOptionsImgs[i, 1] : invOptionsImgs[i, 0], optionsIconsLeft + optionsIconWidth * i, optionsIconsTop, optionsIconWidth, optionsIconWidth);
            }
        }
        ...
        void canvas_MouseDown_SendInv(object sender, MouseEventArgs e)
        {
                ...
                for (int i = 0; i < UsersHandler.UsersCount; i++)
                {
                    ...
                    if (UsersHandler.getUserAtIndex(i).markedInForm)
                    {
                        if (UsersHandler.getUserAtIndex(i).hasChessRunning)
                        {
                            ...
                            UsersHandler.getUserAtIndex(i).invitation_I_SentToOponentOptions.copyInvOptions(defaultInvOptions);
            ...
            else if (e.Y > optionsIconsTop && e.Y < optionsIconsTop + optionsIconWidth)
            {
                ...
                else if (e.X > optionsIconsLeft + optionsIconWidth * 3 && e.X < optionsIconsLeft + optionsIconWidth * 7)
                {
                    ...
                    {
                        defaultInvOptions.TimePrefferedIn = TimeInvOptions.time15min;
                    }
                    else if (e.X < optionsIconsLeft + optionsIconWidth * 5)
                    {
                        defaultInvOptions.TimePrefferedIn = TimeInvOptions.time30min;
                    }
                    else if (e.X < optionsIconsLeft + optionsIconWidth * 6)
                    {
                        defaultInvOptions.TimePrefferedIn = TimeInvOptions.time45min;
                    }
                    else
                    {
                        defaultInvOptions.TimePrefferedIn = TimeInvOptions.time60min;
                    }
                }
                // test if clicked the noTime option
                else if (e.X > optionsIconsLeft + optionsIconWidth * 7 && e.X < optionsIconsLeft + optionsIconWidth * 8)
                {
                    defaultInvOptions.TimePrefferedIn = TimeInvOptions.noTime;
                }
            }
